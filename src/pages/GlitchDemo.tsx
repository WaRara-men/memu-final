...
export default function GlitchDemo() {
  ...
  const runDemo = async () => {
      // 1. Memorize Phase
      setMode('memorize');
      const text1 = "User is a legend hacker.";
      for (let i = 0; i <= text1.length; i++) {
          setQuery(text1.slice(0, i));
          await new Promise(r => setTimeout(r, 50));
      }
      await handleSearch({ preventDefault: () => {} } as React.FormEvent);
      
      await new Promise(r => setTimeout(r, 3000)); // Wait for speak & anim

      // 2. Retrieve Phase
      setMode('retrieve');
      const text2 = "Who is the user?";
      for (let i = 0; i <= text2.length; i++) {
          setQuery(text2.slice(0, i));
          await new Promise(r => setTimeout(r, 50));
      }
      await handleSearch({ preventDefault: () => {} } as React.FormEvent);
  };
  ...
  return (
    <div className={`relative w-full h-screen bg-black text-cyan-400 font-mono overflow-hidden ${memoryLevel === 2 ? 'chaos-mode' : ''} ${isGlitching && memoryLevel === 2 ? 'chaos-flash' : ''}`}>
      {/* 3D Scene Background */}
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-between p-8 pointer-events-none">
        ...
        <div className="w-full max-w-xl pointer-events-auto pb-8 flex flex-col gap-4">
            ...
            <div className="flex justify-center gap-4">
                <button 
                    onClick={runDemo}
                    className="flex items-center gap-2 px-4 py-2 rounded border bg-purple-900/50 border-purple-400 text-purple-300 shadow-[0_0_10px_rgba(128,0,255,0.3)] hover:bg-purple-800/50 transition-all"
                >
